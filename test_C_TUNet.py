...
def main():
    ...
    with torch.no_grad():
        ...
        for i, batch in enumerate(val_loader):
            ...
            val_outputs = sliding_window_inference(val_inputs, (96, 96, 96), 4, model, overlap=args.infer_overlap, mode="gaussian")
            batch["pred"] = val_outputs 
            batch = [post_transform(i) for i in data.decollate_batch(batch)] 
            print("Done")      

            # have the same function with Activationsd and AsDiscreted in post_transforms
            infers = torch.softmax(batch[0]["pred"], 0) 
            infers = torch.argmax(infers, axis=0).cpu().numpy()
            # infers = d[0]["pred"].cpu().numpy()# [0, :, :, :]   

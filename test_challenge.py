...
from networks.unetr_hybrid import UNETR
from trainer import dice, resample_3d
...
# modified by WD
parser.add_argument("--hybrid", action="store_true", help="use hybrid model")

def main():
    args = parser.parse_args()
    args.test_mode = False
    args.test_challenge = True
    ...
    files = load_decathlon_datalist(datalist_json, True, "test", base_dir=data_dir)
    shape = []
    affine = []
    for img_path in files:
        nii_img = nib.load(img_path["image"])
    ...
    with torch.no_grad():
        for i, batch in enumerate(test_loader):
            ...
            img_name = batch["image_meta_dict"]["filename_or_obj"][0].split("/")[-1]
            print("Inference on case {}".format(img_name))
            test_outputs = sliding_window_inference(test_inputs, (96, 96, 96), 4, model, overlap=args.infer_overlap, mode="constant")
            # test_outputs = sliding_window_inference(test_inputs, (96, 96, 96), 4, model, overlap=args.infer_overlap, mode="gaussian")
            ...
            nib.save(
                nib.Nifti1Image(test_outputs.astype(np.uint8), affine[i]), os.path.join(output_directory, img_name)
            )
